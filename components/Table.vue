...
div
    div(v-if="columns().length > 0 || filters.length > 0")
      ...
      #popup(v-html="prettyJSON(md.currentRow)"  ) 
    
      table#mytable
        thead
          th(v-for="col,i in columns()")
            div(@click.exact="sortArray(md.rows, col)") {{ col }}
              span(v-if="i === columns().length-1")
                input.right(type="submit", value="\u29c9copy", v-on:click="selectDom('mytable')")
              <br />
        ...
        tbody
          tr(v-for="row in md.rows")
            td(
              ...
              v-on:mouseover="setCurrentRow(row), setCurrentCol(colnr), showPopup()",
...
import {watch, ref, reactive, onMounted } from 'vue'
  ...
  onMounted( () => {
    ...
    const pages = function() {return Math.round(data.length / ROWSPERPAGE)}
    const setPage = function(pgnr) {
          md.rows = props.selectedBookingsToRender.filter(
            (row, i) =>
              i >= (pgnr - 1) * ROWSPERPAGE && i < pgnr * ROWSPERPAGE
          )
          pageNr = pgnr
        }
    ...
    const selectDom = function (el: any) {
    ...
    const setCurrentCol= function (col) {
          currentCol = col
        }
    const setCurrentRow= function (row) {
          md.currentRow = row
        }
    ...
    const aggregateBy = function (array, aggregateKey) {
    ...
    const euroStringToNumber = (x: string): number => +x.replace("€","").replace(".","").replace(",",".").trim()
    const sumRow = function sum(title) {
          let mySum = 0
    ...
    const sumEuro = function sumEuro(title) {
          let mySum = 0
          if (md.rows === undefined) return ''
          md.rows.forEach(function (row) {
    ...
    const setFilter = function setFilter(title, value, isAntiFilter) {
          ...
          const i = md.filters.findIndex(
            (x) =>
              x.title === title && x.value === value && x.isAnti === isAntiFilter
          )
          ...
          return d.filter(function (row) {
            ...
            filters.forEach(function (filter) {
              if (filter.title === 'pattern') {
                ...
                if (cmpstr.includes(filter.value.toLowerCase()) && filter.isAnti)
                  retval = false
